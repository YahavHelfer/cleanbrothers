...
export const beforeAfterContent = {
  ...
  items: galleryItems,
  ctaLabel: "רוצים לבדוק לפי תמונה? שלחו בוואטסאפ",
};

export function BeforeAfter({ content = beforeAfterContent, preview = false }: {
  content?: typeof beforeAfterContent; preview?: boolean;
} = {}) {
  return (
    <section
      id="before-after"
      ...
      <div className="section-container">
        <div className="rounded-[1.5rem] border theme-inverse-card p-3.5 transition duration-300 hover:border-turquoise/30 sm:rounded-[2rem] sm:p-8">
          ...
          <div className="mt-5 text-center sm:mt-10">
            {preview ? <span className="btn-primary inline-flex min-h-10 px-4 py-2 text-xs sm:min-h-12 sm:px-7 sm:py-3.5 sm:text-base" aria-disabled="true">{content.ctaLabel}</span> : <a
